...
class Server(object):
    ...
    * self.room: one of 0, 1, 2, 3. This signifies which "room" the client is in,
    ...
    def get_input(self):
        ...
        buf = ''.encode('utf8')
        while b'\n' not in buf:
            buf += self.client_connection.recv(16)

        self.input_buffer = buf.decode('utf8').strip()
    ...
    def move(self, argument):
        ...
        * "west"
        ...
        if self.room == 3 and argument == 'south':
            self.room = 0
        elif self.room == 0 and argument == 'north':
            self.room = 3
    ...
    def route(self):
        ...
        :return: None
        """
        args = self.input_buffer.split(" ", maxsplit=1)
        cmd = args[0]
        arg = ''
        if len(args) > 1:
            arg = args[1]
        routes_dict = {'move': self.move, 'quit': self.quit, 'say': self.say}
        routes_dict[cmd](arg)
    ...
    def push_output(self):
        ...
        :return: None
        """
        output_buffer = 'OK! ' + self.output_buffer + '\n'
        self.client_connection.sendall(output_buffer.encode('utf8'))
